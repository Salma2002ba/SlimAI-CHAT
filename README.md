...
- `backend/` : future Python/FastAPI backend (entrypoint to PostgreSQL)
...
- expose a clean API for the `frontend/`,
- connect to PostgreSQL via `DATABASE_URL`,
- encapsulate business logic, data access, and LLM/RAG orchestration.
...
- A minimal `Dockerfile` and `docker-compose.yml` are provided to containerize the frontend and prepare for future services, including PostgreSQL and a future backend service.
